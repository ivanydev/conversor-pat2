...
def process_sheet(df, groups):
    ...
    # Ordenar grupos do mais interno para o mais externo
    # Grupos mais internos têm nomes mais longos (ex: "Grupo 1.1.1" vem antes de "Grupo 1.1")
    groups_sorted = sorted(groups, key=lambda x: len(x["name"]), reverse=True)
    ...
    for group in groups_sorted:
        ...
        start_mask = df["name"].str.strip().replace("\n", "").replace("\r", "") == group["start_field"].strip().replace("\n", "").replace("\r", "")
        ...
        #end_mask = df["name"].str.strip() == group["end_field"].str.strip()
        end_mask = df["name"].str.strip().replace("\n", "").replace("\r", "") == group["end_field"].strip().replace("\n", "").replace("\r", "")
        ...
        df.loc[start_idx - 0.5] = ["begin_group", group["name"].lower().replace(" ", "_"), group["name"], "", "field-list", "", "", "", ""]
